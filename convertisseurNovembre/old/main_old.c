...
#include <inttypes.h>
#include "wincmd.h"
#include <windows.h> // Inutile car déjà inclus dans le wincmd.h : grâce à l'include guard utilisée dans windows.h, cela ne produirait pas d'erreur, mais serait inutile car n'inclurait rien
// Booléens
#define BFALSE 0
#define BTRUE 1
typedef struct {
		   ...
		   case 4 :
			...
			case 5:
				while(choice!=0){
					...
					} clock; // Permet d'économiser de la RAM (? et taille de la struct en ram ??)
					switch(choice){
						case 0: break; // N'affiche pas que quitter le menu est un choix invalide
						case 1:
							printf("\n");
